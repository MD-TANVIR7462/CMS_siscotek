      ...
      >
        <div className="rounded-md shadow-xl overflow-hidden border bg-white dark:bg-gray-900 dark:border-gray-700 transition-colors duration-300">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                rightElement={
                  <button type="button" onClick={() => setShowPassword(!showPassword)}>
                    {showPassword ? (
                      <EyeOff className="h-5 w-5 text-gray-500 dark:text-gray-400 hover:text-gray-600 transition-colors duration-150" />
                    ) : (
                      <Eye className="h-5 w-5 text-gray-500 dark:text-gray-400 hover:text-gray-600 transition-colors duration-150" />
                    )}
                  </button>
                }
              />
              ...
              <div>
                <label className="flex items-center gap-3 text-sm font-medium mb-1 text-black dark:text-gray-200">
                  ...
                  <button type="button" onClick={generateCaptcha} className="text-indigo-500 hover:text-indigo-700 dark:hover:text-indigo-400 transition-colors">
              ...
              >
                {isLoading ? <Spinner /> : <>Sign in <ArrowRight className="ml-2 h-4 w-4" /></>}
              </motion.button>
            </form>
          </div>
          ...
          <div className="px-8 py-4 bg-gray-200 dark:bg-gray-800 text-center border-t border-gray-300 dark:border-gray-700">
            <p className="text-xs text-gray-500 dark:text-gray-400">© {new Date().getFullYear()} CMS. All rights reserved by SiSCOTEK.</p>
          </div>
        </div>
      </motion.div>
    </div>
